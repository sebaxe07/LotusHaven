...
<div class="p-6 bg-gray-100 rounded-xl shadow-lg">
  <div class="flex items-center mb-5">
    <div class="mr-4 text-teal-600 pt-1">
      <UiSvgIcon :icon="getIconPath('professor')" class="w-30 h-30 mt-1 text-teal-500" />
    </div>
    <NuxtLink :to="'/teacher/' + schedule.professor?.id"
      :class="['px-4 py-2 text-lg font-medium rounded-md', schedule.professor?.name ? 'bg-teal-200 bg-opacity-70 text-gray-800' : 'bg-gray-200 text-gray-600']">
  ...
  <div class="space-y-4 text-gray-700">
    <div class="flex items-start">
      <div class="mr-4 text-teal-600 shrink-0 pt-1">
        <UiSvgIcon :icon="getIconPath('calendar')" class="w-30 h-30 mt-1 text-teal-500" />
      </div>
      <div>
    ...
    <div class="flex items-center" v-if="schedule.professor?.phone">
      <div class="mr-4 text-teal-600 shrink-0">
        <UiSvgIcon :icon="getIconPath('phone')" class="w-30 h-30 mt-1 text-teal-500" />
      </div>
      <a :href="`tel:${schedule.professor.phone}`" class="text-md hover:text-teal-700 hover:underline">{{
        schedule.professor.phone }}</a>
    </div>

    <div class="flex items-center" v-if="schedule.professor?.email">
      <div class="mr-4 text-teal-600 shrink-0">
        <UiSvgIcon :icon="getIconPath('email')" class="w-30 h-30 mt-1 text-teal-500" />
      </div>
      <a :href="`mailto:${schedule.professor.email}`" class="text-md hover:text-teal-700 hover:underline"
